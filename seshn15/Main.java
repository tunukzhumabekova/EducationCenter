...
students1(students);
calculateYearsStudying(students);
    }
    ...
    public static void calculateYearsStudying(Student[] students) {
        LocalDate currentDate = LocalDate.now();

        for (Student stu : students) {
            LocalDate startDate = stu.getDataOfStart();
            Period period = Period.between(startDate, currentDate);
            int years = period.getYears();

            System.out.println(stu.getName() + " " + stu.getSurname() + " is studying for " + years + " years.");
        }
